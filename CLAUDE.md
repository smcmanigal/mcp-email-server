...
- **Execution**: `apply_rules()` dispatches one handler per rule. `EmailClient.apply_filter_rule()` opens a **single IMAP connection** per rule, runs batched OR queries (default 10 values/chunk via `_build_or_criteria()`, configurable per-account via `EmailSettings.search_chunk_size`), deduplicates UIDs, then batch-moves (80 UIDs/batch) using MOVE (RFC 6851) with COPY+DELETE fallback. Fail-fast on batch error.
- **`search_chunk_size`** (per-account, default 10): Caps OR-tree depth per IMAP search query. Office 365 silently fails (returns NO/BAD or empty results) when the OR tree exceeds ~10 operands; lower this for stricter servers. Set in TOML or via `MCP_EMAIL_SERVER_SEARCH_CHUNK_SIZE` env var. Non-OK responses from `_search_by_field` are logged at WARNING with a hint to lower this value.
